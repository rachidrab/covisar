...
import com.fdmpartners.covisar.config.KafkaProperties;
...
class CovisarKafkaResourceIT {
    ...
    void setup() {
        ...
        CovisarKafkaResource kafkaResource = new CovisarKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/covisar-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/covisar-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
